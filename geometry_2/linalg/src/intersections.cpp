...
#include "../include/primitives/segment.h"
#include "../include/primitives/triangle.h"
#include "../include/primitives/plane.h"
#include "../include/primitives/distance.h"
#include "../include/primitives/intersections.h"
...
bool lingeo::intersection(const segment_t<3>& s, const point_t<3>& p) {
    ...
    bool flag;
    for(std::size_t k = 0; k < 3lu; ++k) {
        flag = true;
        for(std::size_t i = 0; i < 3lu; ++i) {
            flag = flag && (equal(p[i], s.get_fst()[i] + t[k] * s.get_scd()[i]));
        }
