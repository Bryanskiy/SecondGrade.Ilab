 ...
 *   ------------------------------------------------------------
 *   text     (READ_ONLY)  - clear
 *   patterns (READ ONLY)  - concatenated patterns array
 *   indices  (READ ONLY)  - indices[i] - start of i + 1 pattern
 *   preffix  (READ ONLY)  - concatenated preffix functions array
 *   answer   (WRITE ONLY) - concatinated arrays with positions of matches for every pattern 
 */

__kernel void kmp1(__global char* text, ulong text_size, __global char* patterns, __global ulong* indices, __global ulong* preffix, __global ulong* answer) {
