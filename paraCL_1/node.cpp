...
Inode_t* make_unary_op(unary_op op, Inode_t* node) {make_unary_op(node, op);}
...
Inode_t::return_type_t bin_op_t::calc() {
    int ret = 0;
    switch(op_) {
        case bin_op::plus_:
...
Inode_t::return_type_t unary_op_t::calc() {
    int ret;
    switch(op_) {
...
Inode_t::return_type_t if_t::calc() {
    if(condition_->calc()) {
        scope_->calc();
    }
...
Inode_t::return_type_t while_t::calc() {
