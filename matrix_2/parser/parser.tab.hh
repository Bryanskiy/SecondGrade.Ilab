...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::END_OF_FILE || tok == token::YYerror || tok == token::YYUNDEF || tok == token::EDGE || tok == token::COMMA || tok == token::SCOLON || tok == token::V || tok == token::LBREAK);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_DOUBLE (float v, location_type l)
      {
      ...
      static
      symbol_type
      make_DOUBLE (const float& v, const location_type& l)
      {
      ...
      static
      symbol_type
      make_LBREAK (location_type l)
      {
        return symbol_type (token::LBREAK, std::move (l));
      }
#else
      static
      symbol_type
      make_LBREAK (const location_type& l)
      {
        return symbol_type (token::LBREAK, l);
      }
#endif
