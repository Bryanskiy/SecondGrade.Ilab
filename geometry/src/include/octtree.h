...
namespace octt {
    ...
    private:
        ...
        static int V_min_; // V of elementary space area
    };
...
namespace octt {
    ...
    private:
        bool check_elementary(const T& elem) const;
        bool check_intersection(const T& elem) const;

        std::array<node_t<T> *, 8> children_;
        std::vector<std::pair<bool,T>> elements_;
        space_t space_;
    };
}
...
template<typename T>
void octt::node_t<T>::insert(const T &elem) {
    if(space_.get_Vmin() > space_.V()) {
        bool flag = check_elementary(elem);
        elements_.push_back({flag, elem});
    }

    std::pair<long double, long double> proj[3];
    proj[0] = elem[0].projection_i(0);
    proj[1] = elem[0].projection_i(1);
    proj[2] = elem[0].projection_i(2);
    ...
    if((proj[0].second < x_mid) && (proj[1].second < y_mid) && (proj[2].second < z_mid)) {
        ...
        bool flag = check_elem_intersection(elem);
        elements_.push_back({flag, elem});
        return;
    }

    if(children_[idx] == nullptr) {
        children_[idx] = new space_t(new_space);
    }
...
void octt::node_t<T>::print_intersection(std::ostream& out) const {
    for(std::size_t i = 0, max = elements_.size(); i < max; ++i) {
        if(elements_[i].first == true) {
            out << elements_[i] << std::endl;
        }
    }

    for(std::size_t i = 0, max = children_.size(); i < max; ++i) {
        if(children_[i] != nullptr) {
            children_[i]->print_intersection();
        }
    }
}
...
template<typename T>
bool octt::node_t<T>::check_elementary(const T& elem) const {
    if(elements_.size() == 0) {
        return false;
    }
    ...
    bool flag = false;
    for(std::size_t i = 0, max = elements_.size(); i < max; ++i) {
        if(ivkg::intersection(elements_[i].second, elem)) {
...
template<typename T>
bool octt::node_t<T>::check_intersection(const T &elem) const {
    if(space_.get_Vmin() > space_.V()) {
...
template<typename T>
void octt::octtree_t<T>::print_intersections() const {
    root->print_intersection();
}

template<typename T>
void octt::octtree_t<T>::insert(const T &elem) {
    root->insert(elem);
}
